...
String streamName(String name) {
  if(name.startsWith("_")){
    return name.substring(1).replaceFirst('Controller', 'Stream');
  }
  return name.replaceFirst('Controller', 'Stream');
}

String updateName(String name){
  if(name.startsWith("_")){
    name = capitalize(name.substring(1).replaceFirst('Controller', ''));
  }
  name = capitalize(name.replaceFirst('Controller', ''));
  return "update$name";
}
